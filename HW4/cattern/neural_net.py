...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss_noReg = -np.log(softmax[range(N),y])
    loss_noReg = np.sum(loss_noReg)/N
    reg_term = reg * (np.sum(np.square(W1)) + np.sum(np.square(W2)))
    loss = np.squeeze(loss_noReg + reg_term)
    ...
    #############################################################################
    loss_grad = softmax
    loss_grad[range(N), y] -= 1 # Only correct y column get minus 1 (pi - yi)
    dL = (1/N) * (loss_grad) 
    grads['W2'] = np.dot(a1.T, dL) + (reg*2)*W2 # Chain rule
    grads['b2'] = np.sum(dL, axis=0, keepdims=True) # Chain rule
    da1 = np.dot(dL, W2.T) # Chain rule
    da1[a1 <= 0] = 0 # Derivative of ReLU (1 if value > 0)
    grads['W1'] = np.dot(X.T, da1) + (reg*2)*W1 # Chain rule
    grads['b1'] = np.sum(da1, axis=0, keepdims=True) # Chain rule
    #############################################################################
    #                              END OF TODO#3                                #
    #############################################################################
